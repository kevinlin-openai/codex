...
`app/list` returns after both accessible apps and directory apps are loaded. If the directory request fails but the independently loaded accessible-app inventory succeeds, the response falls back to accessible apps only. Set `forceRefetch: true` to bypass app caches and fetch fresh data from sources. Cache entries are only replaced when those refetches succeed.
